        ...
        if (config['action'] === 'value') {
            ...
            for (let i=0; i<config['data'].length; i++) {
                ...
                if (config['data'][i] === 'api') {
                    items[config['data'][i]] = document.querySelector(`.${config['target']} > .crypto-modal > .modal-group > .modal-gecko > .modal-${config['data'][i]}`).value;
                }

                /** the rest. */
                else {
                    items[config['data'][i]] = document.querySelector(`.${config['target']} > .crypto-modal > .modal-group > .modal-${config['data'][i]}`).value;
                }
            ...
            for (let key in config['data']) {
                if (config['data'].hasOwnProperty(key)) {
                    ...
                    if (config['data'][key] === 'api') {
                        document.querySelector(`.${config['target']} > .crypto-modal > .modal-group > .modal-gecko > .modal-${config['data'][key]}`).value = '';
                    }
        ...
        if (config['action'] === 'populate') {
            ...
            for (let i=0; i<config['data'].length; i++) {
                ...
                /** set api value. */
                    ...
                    document.querySelector(`.${config['target']} > .crypto-modal > .modal-group > .modal-gecko > .modal-${config['data'][i]}`).value = content;
                }
